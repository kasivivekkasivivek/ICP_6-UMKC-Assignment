...
import "./index.css"
import ResturantsItem from "../ResturantsItem"
...
class NearMe extends Component {
    ...
    getResturantsNearMe = async()=>{
        ...
        const response = await fetch(`https://api.foursquare.com/v2/venues/search?ll=40.7,-74&client_id=2DIUNAM5MKLOWND04WECHXBFJPSOB3WAE4JR12MDOKMRAQWB&client_secret=U1ILHE5XGNWJUGUT2WBKVETKXMSVZ3QKJMZ1OWXVQBK0RI0S&v=20211001&categoryId=4d4b7105d754a06374d81259&near=${LocationInput}`);
       ...
       //console.log(ResturantsList);
         return(
             <div className="near_me_container">
                 <h1 className="top_resturants"> Get Top Restaurants Near You !!</h1>
                 <div className="search_container">
                    <input className="input_search_box" type="search" placeholder="Enter your City" 
                    onChange={this.userLocationInput}
                    value={LocationInput}/>

                <button type="button" className="search_button" onClick={this.getResturantsNearMe}>Search</button>
                </div>


                <ul className="resturant_items_container">
                    {ResturantsList.map(eachItem=>(
                        <ResturantsItem item={eachItem} key={eachItem.id}/>
                ))}
                </ul>
             </div>
         )
